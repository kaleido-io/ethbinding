...
func TestHexToAddress(t *testing.T) {
	assert := assert.New(t)
	assert.Equal("0x0123456789abcDEF0123456789abCDef01234567", EthAPIShim.HexToAddress("0x0123456789abcDEF0123456789abCDef01234567").String())
}

func TestBytesToAddress(t *testing.T) {
	assert := assert.New(t)
	b, _ := EthAPIShim.HexDecode("0x0123456789abcDEF0123456789abCDef01234567")
	assert.Equal("0x0123456789abcDEF0123456789abCDef01234567", EthAPIShim.BytesToAddress(b).String())
}

func TestHexToHash(t *testing.T) {
	assert := assert.New(t)
	assert.Equal("0x49d0d5ec93185ca4cd24efff28fdeef97bd06fe7a26f771e645892433184af13", EthAPIShim.HexToHash("0x49d0d5ec93185ca4cd24efff28fdeef97bd06fe7a26f771e645892433184af13").String())
}

func TestABITypeFor(t *testing.T) {
	assert := assert.New(t)
	abiType, _ := EthAPIShim.ABITypeFor("uint256")
	assert.Equal("uint256", abiType.String())
}

func TestABITypeKnown(t *testing.T) {
	assert := assert.New(t)
	assert.Equal("uint256", EthAPIShim.ABITypeKnown("uint256").String())
}
...
func TestABISignature(t *testing.T) {
	...
	ev := abi.NewEvent("test", "test", true, ABIArguments{ABIArgument{
		Name: "arg1",
		Type: EthAPIShim.ABITypeKnown("uint256"),
	}})
	assert.Equal("test(uint256)", EthAPIShim.ABIEventSignature(&ev))
...
func TestRABIMarshalingToABIRuntime(t *testing.T) {
	...
	runtimeABI, err := EthAPIShim.ABIMarshalingToABIRuntime(marshalableABI)
	assert.NoError(err)
...
func TestABIElementMarshalingToABIEvent(t *testing.T) {
	...
	_ = json.Unmarshal([]byte(eventBytes), &input1)
	marshalable := &ABIElementMarshaling{
		Name:   "testevent",
		Inputs: []ABIArgumentMarshaling{input1},
	}

	event, err := EthAPIShim.ABIElementMarshalingToABIEvent(marshalable)
	assert.NoError(err)
...
func TestABIElementMarshalingToABIMethod(t *testing.T) {
	...
	method, err := EthAPIShim.ABIElementMarshalingToABIMethod(marshalable)
	assert.NoError(err)
...
func TestABIElementMarshalingToABIMethodFail(t *testing.T) {
	...
	_, err := EthAPIShim.ABIElementMarshalingToABIMethod(marshalable)
